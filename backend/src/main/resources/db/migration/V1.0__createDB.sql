...
create table review (movie_id bigint not null, user_id varchar(255) not null, date timestamp, rating integer not null check (rating>=1 AND rating<=5), review_text varchar(1000), primary key (movie_id, user_id));
